...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        #################text_mask#################
        # text = torch.bmm(x.transpose(0,1),x.transpose(0,1).transpose(1,2))
        # new_text = F.softmax(text,dim=0)

        # torch.sort(new_text)
        # encoder_padding_mask = self.getBinaryTensor(new_text, 0.02)



        # gating_data = sorted(new_text , dim=-1)[-15]
        # self.getBinaryTensor(new_text, gating_data)
