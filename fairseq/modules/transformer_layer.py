...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.cross_text_encoder = TransformerEncoderLayer_text(args)
    ...
    def mask(self, x, src_img_features, lay_idx):
        ...
        mask_img = torch.bmm(src_img_features, x.transpose(1, 2)) / math.sqrt(512)
        ...
        mask_txt = torch.bmm(x, src_img_features.transpose(1, 2)) / math.sqrt(512)
        ...
        mask_reverse = torch.bmm(mask_txt,mask_img).cuda()
        mask_reverse_output = self.getBinaryTensor(mask_reverse, 0.02)
        ...
        return mask_matrix_output.detach(),mask_reverse_output.detach()

    # def mask_text(self,x):
    #
        #######  mask  text ######### batch * len * 2048
        # mask_text = torch.bmm(x, x.transpose(1, 2))
        # mask_text = F.softmax(mask_text, dim=-1)
        # sorted(mask_text)
    ...
    def forward(self, x, src_img_features, encoder_padding_mask, encoder_padding_mask_image, batch_len, lay_idx,
        ...
        # src_text_mask = torch.zeros(x.size(1), x.size(0), x.size(0)).eq(1)
        mask_matrix,reverse_mask = self.mask(x, src_img_features, lay_idx)
        ...
        if lay_idx >=3 :
            # src_text_mask = torch.zeros(x.size(1), x.size(0), x.size(0)).eq(1)
            x = self.cross_text_encoder(
            x, src_img_features, encoder_padding_mask, reverse_mask
        )
        ...
        if lay_idx <= 2:
            ...
            src_img_mask = torch.zeros(src_img_features.size(1), src_img_features.size(0), src_img_features.size(0)).eq(1)
            src_img_features = self.image_encoder(lay_idx, src_img_features, encoder_padding_mask_image, src_img_mask)
            
        if lay_idx >= 3:
            encoder_padding_mask_image = torch.sum(src_img_features, dim=-1).eq(0).transpose(0, 1)
            src_img_features = self.image_encoder(lay_idx, src_img_features, encoder_padding_mask_image,mask_matrix)



        ########  gating ########
        src_img_features_tmp = src_img_features
        # torch.sum(src_img_features_tmp,x)
        # src_img_features_cat = torch.cat((x, src_img_features_tmp), dim=0)
        # m = nn.Linear(src_img_features_cat.cuda().size(0), x.cuda().size(0))
        # a = src_img_features_cat.view(src_img_features_cat.size(0), -1)
        # b = x.view(x.size(0), -1)
        # x = m(a,b)
        # a,b,c = x.shape
        # src_img_features_cat = torch.cat((x, src_img_features_tmp), dim=0)
        # x = Linear(src_img_features_cat, a)
        if lay_idx >= 2:
            src_img_features, gate = self.gating(x, src_img_features)
            x = x + src_img_features
        return x, mask_matrix.eq(0), src_img_features_tmp

def Linear_method(a,b):
        number = Linear(a.size(0), b.size(0))
        return number
...
class sum(nn.Module):
    ...
    def forward(self, x, src_img_features):
        sum = x + torch.mean(src_img_features,dim=0,keepdim=True).expand(x.size(0),x.size(1),x.size(2))
        # a,b,c = x.shape
        # # cat = self.fc_img(cat)
        # q,w,e = cat.shape
        # linear = Linear(q,a)
        # cat = linear(cat.cpu().permute(2,1,0)).permute(2,1,0).cuda()
        return sum
